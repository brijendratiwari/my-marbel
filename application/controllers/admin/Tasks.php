               ...
               $effort_division_value=$this->input->post('cd-value')/$this->input->post('cd-effort');
                ...
                if ($this->db->insert_id() > 0) {
                    ...
                    if($this->input->post('cd-duedate')){
                     ...
                     $this->db->insert('m_calendar', array('title'=>$this->input->post('cd-taskname'),'event_created_by'=>$this->session->userdata['marbel_user']['user_id'],'event_created_to'=>$assignee,'startdate'=>$start_date_time,'enddate'=>$start_date_time,'start_date'=>$date_time,'end_date'=>$date_time,'allDay'=>'false','event_type'=>'2'));
                    }
                    $result['result'] = TRUE;
