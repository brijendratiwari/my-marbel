<?php
if (isset($_POST['cd-address'], $_POST['cd-order-number'], $_POST['cd-address2'], $_POST['cd-city'], $_POST['cd-state'], $_POST['cd-zip'], $_POST['cd-country'], $_POST['cd-wheel-color'], $_POST['cd-wheel-size'])) {
	$order_number = $_POST['cd-order-number'];
	$orderId = $_POST['cd-order-id'];
	...
	$error = updateOrder($_SESSION['marbel_user']['user_id'], $orderId, $order_number, $delivery_address, $delivery_address_2, $city, $state, $zip, $country, $wheel_color, $wheel_size, $db);
}
...
<section id="main-content">
	<section class="wrapper">
		...
		if (isset($error)) {
			if ($error == 0) {
				echo '<div id="error" class="alert alert-success text-center" style="margin: 30px"><p>Shipping information for order #'.$order_number.' has been updated</p></div></fieldset>';
			} else if ($error == 1) {
				echo '<div id="error" class="alert alert-danger text-center" style="margin: 30px"><p>Could not update shipping for order #'.$order_number.'<br />This item has already shipped or was refunded</p></div></fieldset>';
			} else if ($error == 2) {
				echo '<div id="error" class="alert alert-danger text-center" style="margin: 30px"><p>Could not update shipping for order #'.$order_number.'.<br />Unknown Error</p></div></fieldset>';
			}
		}
		?>
				...
				<div class="form-group">
					...
					<div class="col-md-3"><select name="orders" id="orders" class="form-control" style="padding: 5px; margin: 0px;">';
						...
						foreach ($orders as $order) {
							...
							echo '<option '.((strcmp($country, '') !== FALSE) ? 'selected="selected"' : '').' value="order-'.$order['order_number'].'">'.$order['order_number'].'</option> ';
						}
