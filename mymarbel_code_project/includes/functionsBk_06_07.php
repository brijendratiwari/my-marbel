...
/** 
  ...
  */
	...
	function sec_session_start() {
		$session_name = 'my_marbel';
		$secure = SECURE;
		$httponly = true;
		if (ini_set('session.use_only_cookies', 1) === FALSE) {
			header("Location: /?err=Could not initiate a safe session");
			exit();
		}
...
/**
	...
	*/
	...
	function insertUser($email, $first_name, $last_name, $type, $phone, $notes, $password, $salt, $db) {
		if ($stmt = $db->prepare("SELECT id FROM m_users WHERE email = ? LIMIT 1")) {
			...
			$stmt->fetch();
			if ($stmt->num_rows == 0) {
				$db->query(sprintf("INSERT INTO m_users (email, first_name, last_name, type, register_date, last_activity, phone, notes) VALUES ('%s', '%s', '%s', '%s', UNIX_TIMESTAMP(NOW()), UNIX_TIMESTAMP(NOW()), '%s', '%s');", mysqli_real_escape_string($db, $email), mysqli_real_escape_string($db, $first_name), mysqli_real_escape_string($db, $last_name), $type, mysqli_real_escape_string($db, $phone), mysqli_real_escape_string($db, $notes)));
				$stmt->execute();
				$stmt->store_result();
	...
	function adminUpdateCustomer($user_id, $first_name, $last_name, $type, $email, $phone, $password, $salt, $notes, $db) {
		$db->query(sprintf("UPDATE m_users SET email = '%s', first_name = '%s', last_name = '%s', type = '%s', phone = '%s', notes = '%s' WHERE id = '%s';", mysqli_real_escape_string($db, $email), mysqli_real_escape_string($db, $first_name), mysqli_real_escape_string($db, $last_name), $type, mysqli_real_escape_string($db, $phone), mysqli_real_escape_string($db, $notes), $user_id));
	...
	function getCustomer($id, $db) {	        
		$customer = array();
		if ($stmt = $db->prepare("SELECT mu.id, mu.email, mu.first_name, mu.last_name, mu.type, mu.register_date, mu.last_activity, mu.phone, mu.notes, muli.ip FROM m_users mu LEFT JOIN m_user_login_ip muli ON muli.user_id = mu.id AND muli.time = mu.last_activity WHERE mu.id = ? ORDER BY id ASC;")) {
